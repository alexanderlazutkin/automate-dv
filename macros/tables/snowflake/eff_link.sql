...
{%- macro eff_link(src_pk, src_fk, src_extra_columns, effective_date_from, effective_date_to, src_ldts, src_source, source_model) -%}
    ...
    {{ adapter.dispatch('eff_link', 'automate_dv')(src_pk=src_pk, src_fk=src_fk,
...
{%- macro default__eff_link(src_pk, src_fk, src_extra_columns, effective_date_from, effective_date_to, src_ldts, src_source, source_model) -%}

{%- set source_cols = automate_dv.expand_column_list(columns=[src_pk, src_fk, src_extra_columns, effective_date_from, effective_date_to, src_ldts, src_source]) -%}
...
row_rank_{{ source_number }} AS (
    ...
    QUALIFY ROW_NUMBER() OVER(
        PARTITION BY {{ automate_dv.prefix([src_pk, effective_date_from, effective_date_to], 'rr') }}
...
row_rank_union AS (
    ...
    QUALIFY ROW_NUMBER() OVER(
        PARTITION BY {{ automate_dv.prefix([src_pk, effective_date_from, effective_date_to], 'ru') }}
...
records_to_insert AS (
    ...
    AND {{ automate_dv.multikey(effective_date_from, prefix=['a','d'], condition='=') }}
    AND {{ automate_dv.multikey(effective_date_to, prefix=['a','d'], condition='=') }}
